...
const DEFAULT_MARKERS = [
  { lat: 42.65, lng: -73.75, label: "Albany, NY" },
...
const DEFAULT_CONNECTIONS: { from: [number, number]; to: [number, number] }[] = [
  { from: [42.65, -73.75], to: [12.97, 77.59] },
  { from: [42.65, -73.75], to: [37.78, -122.42] },
  { from: [42.65, -73.75], to: [51.51, -0.13] },
  { from: [42.65, -73.75], to: [40.71, -74.01] },
  { from: [12.97, 77.59], to: [1.35, 103.82] },
  { from: [12.97, 77.59], to: [28.61, 77.21] },
...
export function InteractiveGlobe({
  className,
  size = 500,
  dotColor = "rgba(var(--globe-dot), ALPHA)",
  arcColor = "rgba(var(--globe-arc), 0.5)",
  markerColor = "rgba(var(--globe-marker), 1)",
  ...
  const getColors = useCallback(() => {
    ...
    // Parse HSL values
    const [h, s, l] = accent.split(" ").map((v) => parseFloat(v));

    const dotBase = isDark ? [140, 200, 255] : [60, 100, 160];
    const arcBase = isDark ? [100, 180, 255] : [80, 140, 200];
    const markerBase = isDark ? [100, 220, 255] : [40, 120, 200];

    return {
      dot: (alpha: number) => `rgba(${dotBase.join(",")},${alpha})`,
      arc: `rgba(${arcBase.join(",")},0.45)`,
      marker: `rgba(${markerBase.join(",")},1)`,
      markerFade: (alpha: number) => `rgba(${markerBase.join(",")},${alpha})`,
      glow: isDark ? "rgba(60,140,255,0.03)" : "rgba(60,100,180,0.02)",
      ring: isDark ? "rgba(100,180,255,0.06)" : "rgba(60,120,180,0.08)",
    };
  }, []);
  ...
  const draw = useCallback(() => {
    ...
    for (let i = 0; i < dots.length; i++) {
      ...
      ctx.fillStyle = colors.dot(depthAlpha);
      ctx.fill();
    }
    ...
    for (const marker of markers) {
      ...
      ctx.strokeStyle = colors.markerFade(0.2 + pulse * 0.15);
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = colors.markerFade(0.6);
        ctx.fillText(marker.label, sx + 8, sy + 3);
      }
    }

    animRef.current = requestAnimationFrame(draw);
  }, [getColors, autoRotateSpeed, connections, markers]);
  ...
  return (
    <canvas
      ...
      style={{ width: `min(100%, ${size}px)`, height: `min(100%, ${size}px)` }}
